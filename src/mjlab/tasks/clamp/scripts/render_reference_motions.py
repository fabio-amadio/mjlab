"""Render one reference video per CLAMP motion clip.

This script does not run a policy. It loads a CLAMP play environment, then
poses the robot directly on the reference motion and saves one `.mp4` per clip.
For YAML- or directory-based motion sources, the output folder mirrors the
...
class RenderReferenceMotionsConfig:
  """Configuration for batch rendering CLAMP reference motions."""
...
def _write_video(path: Path, frames: list[np.ndarray], fps: float) -> None:
  ...
  clip = ImageSequenceClip([_ensure_uint8(frame) for frame in frames], fps=fps)
  clip.write_videofile(str(path), audio=False, logger=None)
  clip.close()
...
def run(cfg: RenderReferenceMotionsConfig) -> None:
  """Render one video per motion clip for a CLAMP task."""
  ...
  all_motion_files, _ = NpzMotionLibrary._resolve_motion_entries(motion_source)
