...
@pytest.mark.parametrize(
  ...
  [
    (go2_constants.GO2_HIP_ACTUATOR_CFG, go2_constants.STIFFNESS_HIP, go2_constants.DAMPING_HIP),
    (go2_constants.GO2_KNEE_ACTUATOR_CFG, go2_constants.STIFFNESS_KNEE, go2_constants.DAMPING_KNEE),
  ],
)
# fmt: on
...
def test_collision_geom_count(go2_model) -> None:
  ...
  foot_geoms = [
    g.name
    for g in [go2_model.geom(i) for i in range(go2_model.ngeom)]
